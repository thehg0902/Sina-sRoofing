...
| 4     | media        | done    | HUMAN: fill slots/approve | 2026-07-07 |
...
- Phase 4: RESOLVED 2026-07-07 (post-deploy, via /ingest). Operator dropped real media that didn't match the original before/after-pairs + static-hero plan; viewed each file, confirmed the new plan in-chat (general work-portfolio gallery, custom alternating two-clip video hero, real logo), rewrote SHOPPING_LIST.md v2, ran /ingest, then hand-tuned encode settings twice (video CRF for combined page-weight budget, image dimensions for actual grid display size) to clear check.py. Home hero is now a real alternating video (site/script.js, custom crossfade player per operator spec), Gallery page + Home teaser show 6/3 real photos, header logo is the real asset (footer stays text — logo has no contrast on the dark footer bg). See state/DECISIONS.md "Post-deploy media ingest" for full detail. NOT yet committed/redeployed — production still has the pre-media version.
