...
PLACEMENT FOLDERS (v1.5.0): Phase 4 creates one subfolder per
placement, named `<Page> - <Section>` (e.g. `Home - Hero/`,
`Contact - onscreen pictures/`), and each shopping-list block carries a
`folder:` line. Drop each file into its matching folder. Folders are
organizational only — /ingest matches RECURSIVELY by exact filename, so
a file at the slots/ root still works; the same filename in two folders
is reported as a conflict, never guessed.
